    ...
    <div class="card shadow-sm border-0">
        <div class="card-body p-4">
            <form action="{{ route('payment.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <select name="invoice_id" id="invoice_id" class="form-select @error('invoice_id') is-invalid @enderror" required>
                        ...
                        @foreach($invoices as $invoice)
                            @php
                                $totalDibayar = (float) ($invoice->total_dibayar ?? 0);
                                $sisaTagihan = max((float) $invoice->nominal - $totalDibayar, 0);
                            @endphp
                            <option value="{{ $invoice->id }}"
                                ...
                                data-total-dibayar="{{ $totalDibayar }}"
                ...
                <div class="p-3 p-md-4 bg-light rounded border mb-4">
                    <div class="d-flex align-items-center gap-3 border-bottom pb-3 mb-3">
                        ...
                        <div>
                            ...
                            <small class="text-muted">Detail kontrak, klien, dan nilai tagihan invoice terpilih.</small>
                        </div>
                    </div>
                    ...
                    <div class="row g-2 mt-3">
                        <div class="col-md-4">
                            <div class="h-100 bg-white border rounded p-3">
                        ...
                        <div class="col-md-4">
                            <div class="h-100 bg-white border rounded p-3">
                                ...
                                <div class="fw-bold text-dark" id="info_total_dibayar">-</div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="h-100 bg-white border border-primary-subtle rounded p-3">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="number" name="jumlah_pembayaran" id="jumlah_pembayaran" class="form-control @error('jumlah_pembayaran') is-invalid @enderror" min="1" value="{{ old('jumlah_pembayaran') }}" required>
                        <small class="text-muted" id="jumlah_pembayaran_hint">Jumlah pembayaran harus sama dengan nominal invoice.</small>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="kode_billing" id="kode_billing" class="form-control @error('kode_billing') is-invalid @enderror" value="{{ old('kode_billing') }}">
                    </div>
                    <div class="col-md-6 mb-3">
                        <label for="ntpn" class="form-label fw-semibold">NTPN</label>
                        <input type="text" name="ntpn" id="ntpn" class="form-control @error('ntpn') is-invalid @enderror" value="{{ old('ntpn') }}">
                    </div>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const kodeBillingInput = document.getElementById('kode_billing');
        ...
        const infoTotalDibayar = document.getElementById('info_total_dibayar');
        const infoSisaTagihan = document.getElementById('info_sisa_tagihan');
        const jumlahPembayaranHint = document.getElementById('jumlah_pembayaran_hint');
        ...
        function updateInfo() {
            ...
            if (!option || option.value === '') {
                ...
                infoTotalDibayar.textContent = '-';
                infoSisaTagihan.textContent = '-';
                jumlahPembayaranInput.removeAttribute('min');
                jumlahPembayaranInput.removeAttribute('max');
                jumlahPembayaranHint.textContent = 'Jumlah pembayaran harus sama dengan nominal invoice.';
                return;
            }
            ...
            const totalDibayar = option.getAttribute('data-total-dibayar');
            ...
            infoTotalDibayar.textContent = rupiah(totalDibayar);
            infoSisaTagihan.textContent = rupiah(sisaTagihan);
            jumlahPembayaranInput.min = sisaTagihan || '';
            jumlahPembayaranInput.max = sisaTagihan || '';
            jumlahPembayaranHint.textContent = 'Harus dibayar sesuai nominal invoice: ' + rupiah(sisaTagihan);

            if (!kodeBillingInput.value) {
                kodeBillingInput.value = kodeBilling || '';
            }
            if (!jumlahPembayaranInput.value) {
